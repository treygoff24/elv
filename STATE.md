...
## 0.5.0 published
- `eleven-agent-cli@0.5.0` is on npm as `latest` (published 2026-09-21, shasum `be1b9bcf…`, identical to the reproducible local pack); `main` is on Forgejo and GitHub at the release commits. 0.4.0 was never published, so 0.5.0 is the first registry release since 0.3.0. The installed `elv 0.5.0` matches the repo build; repo, installed-package, and active skill files match (`npm run verify:install`).
...
- No `v0.5.0` git tag; Trey's publish authorization did not name a tag, and GitHub tags stay gated.
- Publishing needs an interactive npm login plus a 2FA web tap: `npm login --auth-type=web` links expire after a few minutes of polling, and `npm publish` must run under a pty (`script -qfec`) so npm prints the 2FA URL and polls it. The registry read replica lagged several minutes behind the website after the publish.
...
## Next action
Dogfood the installed CLI and file friction as beads; `elv-vht` (parked review minors) and `elv-706` (retry fixture) are the open follow-ups.
