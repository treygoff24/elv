...
const cases: Array<{ name: string; alias: () => BuiltAlias; call: () => BuiltAlias }> = [
  {
    name: "tts",
    alias: () => buildTtsInput({ voiceId: "voice_1", text: "Hello.", model: "eleven_v3", format: "mp3_44100_128" }),
    call: () => ({
      operationId: "text_to_speech_full",
      input: { path: { voice_id: "voice_1" }, query: { output_format: "mp3_44100_128" }, body: { text: "Hello.", model_id: "eleven_v3" } },
    }),
  },
  {
    name: "tts stream timestamps",
    alias: () => buildTtsInput({ stream: true, timestamps: true, voiceId: "voice_1", textFile: join(dir, "script.txt") }),
    call: () => ({ operationId: "text_to_speech_stream_with_timestamps", input: { path: { voice_id: "voice_1" }, body: { text: "From file" } } }),
  },
  {
    name: "stt",
    alias: () => buildSttInput({ file: audio, model: "scribe_v2", timestamps: "word", diarize: true, language: "en", webhook: "https://example.test/hook" }),
    call: () => ({
      operationId: "speech_to_text",
      input: { files: { file: resolve(audio) }, body: { model_id: "scribe_v2", timestamps_granularity: "word", diarize: true, language_code: "en", webhook: "https://example.test/hook" } },
    }),
  },
  {
    name: "music",
    alias: () => buildMusicInput({ prompt: "30s lo-fi loop", model: "music_v2", format: "mp3_44100_128" }),
    call: () => ({ operationId: "generate", input: { query: { output_format: "mp3_44100_128" }, body: { prompt: "30s lo-fi loop", model_id: "music_v2" } } }),
  },
  {
    name: "music stream",
  ...
  },
  {
    ...
    call: () => ({ operationId: "sound_generation", input: { query: { output_format: "mp3_44100_128" }, body: { text: "Door creak", duration_seconds: 3 } } }),
  },
  {
    name: "voice-change",
    alias: () => buildVoiceChangeInput({ voiceId: "voice_1", file: audio, model: "eleven_english_sts_v2", format: "mp3_44100_128" }),
    call: () => ({ operationId: "speech_to_speech_full", input: { path: { voice_id: "voice_1" }, query: { output_format: "mp3_44100_128" }, files: { audio: resolve(audio) }, body: { model_id: "eleven_english_sts_v2" } } }),
  },
  {
    name: "voice-change stream",
    alias: () => buildVoiceChangeInput({ stream: true, voiceId: "voice_1", file: audio }),
    call: () => ({ operationId: "speech_to_speech_stream", input: { path: { voice_id: "voice_1" }, files: { audio: resolve(audio) } } }),
  },
  {
    name: "voice-isolate",
    alias: () => buildVoiceIsolateInput({ file: audio, fileFormat: "other" }),
    call: () => ({ operationId: "audio_isolation", input: { files: { audio: resolve(audio) }, body: { file_format: "other" } } }),
  },
  {
    name: "dubbing create",
    alias: () => buildDubbingCreateInput({ file: video, source: "en", target: "es", name: "Dub" }),
    call: () => ({ operationId: "create_dubbing", input: { files: { file: resolve(video) }, body: { source_lang: "en", target_lang: "es", name: "Dub" } } }),
  },
  ...
  { name: "usage range", alias: () => buildUsageInput({ from: "2026-06-01", to: "2026-06-25", breakdown: "voice", metric: "credits" }), call: () => ({ operationId: "usage_characters", input: { query: { start_unix: Date.parse("2026-06-01"), end_unix: Date.parse("2026-06-25"), breakdown_type: "voice", metric: "credits" } } }) },
];
...
describe("curated aliases", () => {
  ...
  it.each(cases)("$name builds a request semantically equal to elv call", async ({ alias, call }) => {
  ...
  it("selects tts operation variants from stream/timestamps flags", () => {
    ...
    expect(buildTtsInput({ stream: true, voiceId: "v", text: "x" }).operationId).toBe("text_to_speech_stream");
    expect(buildTtsInput({ timestamps: true, voiceId: "v", text: "x" }).operationId).toBe("text_to_speech_full_with_timestamps");
    expect(buildTtsInput({ stream: true, timestamps: true, voiceId: "v", text: "x" }).operationId).toBe("text_to_speech_stream_with_timestamps");
  });
...
async function comparableRequest(op: OperationCard, input: AgentInput) {
  ...
  return {
    ...
    headers: Object.fromEntries(Object.entries(req.headers).filter(([key]) => !key.toLowerCase().startsWith("content-type"))),
    body: req.body instanceof FormData ? await formMap(req.body) : req.body ? JSON.parse(req.body) : undefined,
  };
}
...
async function formMap(form: FormData): Promise<Record<string, unknown[]>> {
  ...
  for (const [key, value] of form.entries()) {
    const item = value instanceof Blob ? { name: (value as File).name, size: value.size, type: value.type } : value;
